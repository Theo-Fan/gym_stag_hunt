...
def main():
    ...
    print("Start niceness evaluation (stag share) at (GMT): ", start_time)
    print("============================================================================================")
    ...
    for ep in range(test_episodes):
        ...
        for t in range(max_ep_len):
            ...
            if env_r0 > 0:
                ...
                if abs(env_r0 - STAG_REWARD) < 1e-6:
                    total_self_stag += 1
                    ep_self_stag += 1
                elif abs(env_r0 - PLANT_REWARD) < 1e-6:
                    total_self_plants += 1
                    ep_self_plants += 1
                else:
        ...
        if ep_self_positive > 0:
            ep_stag_ratio = ep_self_stag / ep_self_positive
        else:
            ep_stag_ratio = float('nan')
        ...
        print(f"episode niceness (stag / positive): {ep_stag_ratio:.3f}")

    print("\n================ Global niceness (stag share) summary ================")
    ...
    if total_self_positive > 0:
        p_stag = total_self_stag / total_self_positive
        sd = math.sqrt(p_stag * (1 - p_stag))
        se = sd / math.sqrt(total_self_positive)
        half = 1.96 * se

        print(f"\nNiceness metric (stag share) p = {p_stag:.4f}")
        print(f"SD  = {sd:.6f}")
        print(f"SE  = {se:.6f}")
        print(f"95% CI (Wald) ≈ [{p_stag - half:.4f}, {p_stag + half:.4f}]")
    else:
        print("\nAgent0 never got positive reward – cannot compute stag share.")
